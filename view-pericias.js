/**
 * view-pericias.js — PsicoApp SPA
 * Gestión de pericias judiciales: pipeline + lista + CRUD
 */
...
/* ── HEADER ── */
#view-pericias .per-header {
  background: linear-gradient(145deg, #1E3A5F 0%, #1B4F72 55%, #2E86C1 100%);
  padding: 20px 20px 28px;
  position: relative; overflow: hidden;
}
#view-pericias .per-header::after {
  content: ''; position: absolute; right: -40px; top: -40px;
  width: 180px; height: 180px; border-radius: 50%;
  background: rgba(255,255,255,0.05);
}
#view-pericias .per-header-row {
  display: flex; align-items: center; justify-content: space-between;
  position: relative; z-index: 1;
}
#view-pericias .per-title {
  font-family: var(--font-display); font-size: 22px;
  font-weight: 700; color: white;
}
#view-pericias .per-title span { color: #7EC8E3; }
#view-pericias .per-subtitle {
  font-size: 13px; color: rgba(255,255,255,0.65);
  margin-top: 4px; position: relative; z-index: 1;
}
#view-pericias .per-fab {
...
#view-pericias .per-stage-name {
  font-size: 12px; font-weight: 800;
  text-transform: uppercase; letter-spacing: .8px;
}
#view-pericias .per-stage-count {
  font-size: 11px; font-weight: 700; padding: 3px 9px; border-radius: 20px;
}

/* ── CARD ── */
#view-pericias .per-card {
  background: var(--surface); border-radius: 14px;
...
#view-pericias .per-card-top {
  display: flex; align-items: flex-start;
  justify-content: space-between; margin-bottom: 5px;
}
#view-pericias .per-card-exp { font-size: 13px; font-weight: 800; }
#view-pericias .per-card-badge {
  font-size: 10px; font-weight: 700;
  padding: 3px 8px; border-radius: 20px; flex-shrink: 0;
}
#view-pericias .per-card-caratula {
  font-size: 13px; color: var(--text-muted);
  margin-bottom: 6px; white-space: nowrap;
  overflow: hidden; text-overflow: ellipsis;
}
#view-pericias .per-card-meta {
  display: flex; gap: 12px; flex-wrap: wrap;
}
#view-pericias .per-card-meta-item {
  font-size: 11px; color: var(--text-muted);
  display: flex; align-items: center; gap: 4px;
}
#view-pericias .per-alerta {
  background: #FFF3E0; color: #E65100;
  border-radius: 8px; padding: 5px 10px;
  font-size: 11px; font-weight: 700;
  margin-top: 8px; display: flex; align-items: center; gap: 6px;
}
[data-theme="dark"] #view-pericias .per-alerta {
  background: rgba(230,81,0,0.15); color: #FB8C00;
}

/* ── EMPTY ── */
#view-pericias .per-empty {
...
.per-btn-cancel {
  ...
  padding: 14px; font-size: 14px; color: var(--text-muted);
...
/* ── DETALLE ── */
#per-overlay-det .per-det-fila {
  display: flex; gap: 10px; padding: 9px 0;
  border-bottom: 1px solid var(--border);
}
#per-overlay-det .per-det-fila-icon { font-size: 16px; flex-shrink: 0; }
#per-overlay-det .per-det-fila-label {
  font-size: 11px; color: var(--text-muted);
  font-weight: 700; text-transform: uppercase; letter-spacing: .5px;
}
#per-overlay-det .per-det-fila-val {
...
/* Colores de estado */
#view-pericias .per-s1 .per-stage-dot,
#view-pericias .per-s1 .per-stage-count { background: #1976D2; }
...
#view-pericias .per-s2 .per-stage-dot,
#view-pericias .per-s2 .per-stage-count { background: #E65100; }
...
#view-pericias .per-s3 .per-stage-dot,
#view-pericias .per-s3 .per-stage-count { background: #7B5EA7; }
...
#view-pericias .per-s4 .per-stage-dot,
#view-pericias .per-s4 .per-stage-count { background: var(--primary); }
...
/* ══════════════════════════════════════════
   RENDER PRINCIPAL
   ══════════════════════════════════════════ */
(function renderPericiaView() {
  const sec = document.getElementById('view-pericias');
  if (!sec) return;
  ...
  sec.innerHTML = `
    ...
    <div class="per-header">
      <div class="per-header-row">
        ...
        <button class="per-fab" onclick="perAbrirModal()" title="Nueva pericia">＋</button>
      </div>
    </div>
    ...
    <div class="per-stats">
      <div class="per-stat">
        <div class="per-stat-num" id="per-stat-encurso" style="color:#1976D2">—</div>
        <div class="per-stat-label">En curso</div>
      </div>
      <div class="per-stat">
        <div class="per-stat-num" id="per-stat-venc" style="color:#E65100">—</div>
        <div class="per-stat-label">Próx. venc.</div>
      </div>
      <div class="per-stat">
        <div class="per-stat-num" id="per-stat-honorarios" style="color:var(--primary);font-size:16px">$0</div>
        <div class="per-stat-label">Honor. pend.</div>
      </div>
    </div>

    <!-- TABS -->
    <div class="per-tabs">
      <button class="per-tab per-tab-active" id="per-tab-pipeline" onclick="perSetTab('pipeline')">⚖️ Pipeline</button>
      <button class="per-tab" id="per-tab-lista" onclick="perSetTab('lista')">📋 Todas</button>
    </div>
    ...
    <div id="per-overlay">
      <div class="per-modal">
        ...
        <div class="per-two-col">
          <div class="per-field">
            <div class="per-field-label">Expediente <span class="per-field-req">*</span></div>
            <input class="per-field-input" id="per-f-expediente" type="text" placeholder="Nº expediente">
          </div>
          <div class="per-field">
            <div class="per-field-label">Juzgado</div>
            <input class="per-field-input" id="per-f-juzgado" type="text" placeholder="Ej: Civil Nº3">
          </div>
        </div>
        <div class="per-field">
          <div class="per-field-label">Carátula <span class="per-field-req">*</span></div>
          <input class="per-field-input" id="per-f-caratula" type="text" placeholder="Ej: García c/ Empresa SA">
        </div>
        <div class="per-field">
          <div class="per-field-label">Peritado</div>
          <input class="per-field-input" id="per-f-peritado" type="text" placeholder="Nombre del peritado">
        </div>
        <div class="per-two-col">
          <div class="per-field">
            <div class="per-field-label">Fecha entrevista</div>
            <input class="per-field-input" id="per-f-entrevista" type="date">
          </div>
          <div class="per-field">
            <div class="per-field-label">Fecha vencimiento</div>
            <input class="per-field-input" id="per-f-vencimiento" type="date">
          </div>
        </div>
        <div class="per-two-col">
          <div class="per-field">
            <div class="per-field-label">Honorarios</div>
            <input class="per-field-input" id="per-f-honorarios" type="number" placeholder="$0">
          </div>
          <div class="per-field">
        ...
        <div class="per-field">
          <div class="per-field-label">Notas</div>
          <textarea class="per-field-input" id="per-f-notas" placeholder="Observaciones..."
            style="resize:none;min-height:70px;font-size:14px;line-height:1.5"></textarea>
        </div>

        <button class="per-btn-save" id="per-btn-guardar" onclick="perGuardar()">✓ Guardar pericia</button>
        <button class="per-btn-cancel" onclick="perCerrarModal()">Cancelar</button>
      </div>
    </div>
    ...
    <div id="per-overlay-det">
      <div class="per-modal">
        ...
        <div style="margin-top:14px">
          ...
          <select class="per-estado-select" id="per-det-estado" onchange="perCambiarEstado(this.value)">
            <option value="en_curso">⚙️ En curso</option>
        ...
        <div class="per-det-actions">
          <button class="per-btn-del" onclick="perEliminar()">🗑 Eliminar</button>
        </div>
        <button class="per-btn-cancel" onclick="perCerrarDetalle()">Cerrar</button>
      </div>
    </div>
  `;
  ...
  document.getElementById('per-overlay').addEventListener('click', e => {
    if (e.target.id === 'per-overlay') perCerrarModal();
  });
  document.getElementById('per-overlay-det').addEventListener('click', e => {
    if (e.target.id === 'per-overlay-det') perCerrarDetalle();
  });
})();
...
/* ══════════════════════════════════════════
   TABS
   ══════════════════════════════════════════ */
function perSetTab(t) {
  _perTabActual = t;
  ['pipeline','lista'].forEach(tab => {
    document.getElementById('per-tab-' + tab)
      ?.classList.toggle('per-tab-active', tab === t);
  });
  _perRenderTab();
}
...
function _perRenderPipeline() {
  ...
  if (!activas.length) {
    cont.innerHTML = `<div class="per-empty">
      <div class="per-empty-icon">⚖️</div>
      <h3>Sin pericias activas</h3>
      <p>Tocá + para registrar la primera.</p>
    </div>`;
    return;
  }
  ...
  orden.forEach(estado => {
    ...
    html += `<div class="per-stage ${est.cls}">
      <div class="per-stage-header">
        <div class="per-stage-badge">
          <div class="per-stage-dot"></div>
          <div class="per-stage-name">${est.label}</div>
        </div>
...
function _perRenderLista() {
  ...
  if (!_perPericias.length) {
    cont.innerHTML = `<div class="per-empty">
      <div class="per-empty-icon">📋</div>
      <h3>Sin pericias registradas</h3>
    </div>`;
    return;
  }
...
function _perFilaDet(icon, label, val) {
  return `<div class="per-det-fila">
    ...
    <div>
      <div class="per-det-fila-label">${label}</div>
      <div class="per-det-fila-val">${val}</div>
    </div>
  </div>`;
}
...
/* ══════════════════════════════════════════
   HOOK DE NAVEGACIÓN
   ══════════════════════════════════════════ */
window.onViewEnter_pericias = async function() {
  await perCargar();
};
