...
(function () {
  ...
  function _rellenarSelectPaciente() {
    const sel = document.getElementById('ag-f-paciente');
    if (!sel) return;
  ...
  function renderAgenda() {
    ...
    document.getElementById('ag-btn-crear').addEventListener('click', crearTurno);
    document.getElementById('ag-btn-cancelar').addEventListener('click', cerrarModal);
    document.getElementById('ag-overlay').addEventListener('click', e => { if (e.target.id === 'ag-overlay') cerrarModal(); });

    document.getElementById('ag-det-confirmar').addEventListener('click', () => cambiarEstado('confirmado'));
    document.getElementById('ag-det-realizada').addEventListener('click', () => cambiarEstado('realizado'));
    document.getElementById('ag-det-eliminar').addEventListener('click', eliminarTurno);
    document.getElementById('ag-det-cerrar').addEventListener('click', cerrarDetalle);
    document.getElementById('ag-overlay-det').addEventListener('click', e => { if (e.target.id === 'ag-overlay-det') cerrarDetalle(); });
  ...
  // ── MONTH GRID ────────────────────────────────────────────
  function renderMes() {
    const grid = document.getElementById('ag-month-grid');
    if (!grid) return;
    grid.innerHTML = '';
  ...
  function abrirModal(modo, fecha, hora) {
    ...
    document.getElementById('ag-modal-sub').textContent   = esTurno ? 'Agendá un turno con un paciente.' : 'Agregá un evento a tu agenda.';
    document.getElementById('ag-btn-crear').textContent   = esTurno ? '✓ Agendar turno' : '✓ Guardar evento';
  ...
  async function crearTurno() {
    const fecha = document.getElementById('ag-f-fecha').value;
    const hora  = document.getElementById('ag-f-hora').value;
    if (!fecha || !hora) { mostrarError('Fecha y hora son obligatorios.'); return; }

    const btn = document.getElementById('ag-btn-crear');
    btn.disabled = true;
  ...
  function abrirDetalle(id) {
    ...
    document.getElementById('ag-det-header').innerHTML = `
    ...
    document.getElementById('ag-det-confirmar').style.display = t.estado === 'pendiente' ? '' : 'none';
    document.getElementById('ag-det-realizada').style.display = t.estado !== 'realizado'  ? '' : 'none';

    document.getElementById('ag-overlay-det').classList.add('open');
  }
  ...
  function cerrarDetalle() {
    document.getElementById('ag-overlay-det').classList.remove('open');
    _turnoSel = null;
  }
