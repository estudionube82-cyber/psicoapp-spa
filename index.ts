...
// ─────────────────────────────────────────────────────────────────────────────
// Rate limiting — en memoria por IP (persiste mientras el isolate esté caliente)
// Protege contra brute-force del CRON_SECRET y abuso de la función.
// ─────────────────────────────────────────────────────────────────────────────
...
Deno.serve(async (req: Request) => {
  ...
  if (!_checkRateLimit(clientIp)) {
    console.warn(`[recordatorios] Rate limit excedido para IP: ${clientIp}`)
